...
divP n d | n `mod` d == 0 = divP (n `div` d) d
         | otherwise = n
...
isPermutation a b = let xa = toDigits a
                        xb = toDigits b
                     in sort xa == sort xb
...
main = do
    ...
    (print . minimumBy (compare `on` snd) . map (\(a,b) -> (a,a % b)) . filter (uncurry isPermutation) . tail ) =<< (A.getAssocs =<< buildArray lim cache)
