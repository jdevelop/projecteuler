...
import Data.List
import Data.ByteString.Char8 as BS (readFile, lines, pack, unpack, split)
...
calculate :: (Ord b, Read a, Integral b) => Int -> Int -> ( a -> b ) -> [[a]] -> b
calculate w h f src = maximum . concatMap getProducts $ concatMap ( $ matrix ) [ horizontals, verticals, diagonals ]
    where
        matrix = buildMatrix f w h src
     
doJob w h filename = BS.readFile filename >>= print . calculate w h read . map ( map ( unpack) ) . map (split ' ') . BS.lines
